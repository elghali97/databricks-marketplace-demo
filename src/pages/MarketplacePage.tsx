...
const MarketplacePage = () => {
  ...
  const getFeaturedDatasetPerCategory = (datasets: Dataset[], count: number = 3) => {
    const categoriesUsed = new Set<DatasetCategory>();
    const featured: Dataset[] = [];
    
    // Sort all datasets by quality score, rating, and download count to get the best options
    ...
    // Select one dataset per category until we have the desired count
    for (const dataset of sortedDatasets) {
      if (!categoriesUsed.has(dataset.category) && featured.length < count) {
        featured.push(dataset);
        categoriesUsed.add(dataset.category);
      }
    }
  ...
  const featuredDatasets = getFeaturedDatasetPerCategory(mockDatasets, 1);
  const trendingDatasets = getFeaturedDatasetPerCategory(mockDatasets.slice(1), 6);
