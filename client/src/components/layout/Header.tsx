        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="relative">
            <button 
              ...
              className="user-button flex items-center space-x-3 p-2 rounded-xl hover:bg-databricks-oat-light transition-all duration-200 border border-transparent hover:border-databricks-oat-medium"
            ...
            {userMenuOpen && (
              <div className="user-menu absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-xl border border-databricks-oat-medium animate-scale-in z-50">
                {/* Enhanced User Info Header */}
                <div className="p-5 border-b border-databricks-oat-medium bg-gradient-to-r from-databricks-oat-light/30 to-white">
                  <div className="flex items-start space-x-4">
                    <div className="flex-shrink-0">
                      {user?.avatar ? (
                        <img 
                          src={user.avatar} 
                          alt={user.name} 
                          className="h-12 w-12 rounded-xl object-cover ring-2 ring-databricks-lava-200 shadow-sm"
                        />
                      ) : (
                        <div className="h-12 w-12 rounded-xl bg-gradient-to-br from-databricks-lava-500 to-databricks-lava-600 flex items-center justify-center text-white font-bold text-lg shadow-sm">
                          {user?.name.charAt(0)}
                        </div>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="font-bold text-base text-databricks-navy-800 truncate" style={{ fontFamily: 'DM Sans, system-ui, sans-serif' }}>
                        {user?.name}
                      </div>
                      <div className="text-sm text-databricks-navy-600 truncate mt-1" style={{ fontFamily: 'DM Sans, system-ui, sans-serif' }}>
                        {user?.email}
                      </div>
                      <div className="text-xs text-databricks-navy-500 mt-1 truncate" style={{ fontFamily: 'DM Sans, system-ui, sans-serif' }}>
                        {user?.organization}
                      </div>
                    </div>
                  </div>
                ...
                <div className="p-2">
                  <Link 
                    to="/profile"
                    className="flex items-center px-3 py-3 text-sm text-databricks-navy-700 hover:bg-databricks-oat-light rounded-lg transition-colors group"
                    onClick={() => setUserMenuOpen(false)}
                    style={{ fontFamily: 'DM Sans, system-ui, sans-serif' }}
                  >
                    <User className="mr-3 h-4 w-4 text-databricks-navy-600 group-hover:text-databricks-lava-600" />
                  ...
                  <Link 
                    ...
                    style={{ fontFamily: 'DM Sans, system-ui, sans-serif' }}
                  >
                    <Database className="mr-3 h-4 w-4 text-databricks-navy-600 group-hover:text-databricks-lava-600" />
                  ...
                  {user?.role === UserRole.ADMIN && (
                    <Link 
                      to="/admin"
                      className="flex items-center px-3 py-3 text-sm text-databricks-navy-700 hover:bg-purple-50 rounded-lg transition-colors group border border-purple-100"
                      onClick={() => setUserMenuOpen(false)}
                      style={{ fontFamily: 'DM Sans, system-ui, sans-serif' }}
                    >
                      <Shield className="mr-3 h-4 w-4 text-purple-600 group-hover:text-purple-700" />
                  ...
                  <button
                    ...
                    className="w-full flex items-center px-3 py-3 text-sm text-red-600 hover:bg-red-50 rounded-lg transition-colors group"
                    style={{ fontFamily: 'DM Sans, system-ui, sans-serif' }}
                  >
